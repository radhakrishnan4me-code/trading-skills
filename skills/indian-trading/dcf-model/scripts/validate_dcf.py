...
class DCFModelValidator:
    ...
    def _check_wacc_range(self):
        ...
        try:
            ...
            if wacc is not None:
                if wacc < 0.08 or wacc > 0.25:
                    self.warnings.append(
                        f"WACC ({wacc:.2%}) is outside typical range for Indian markets (8%-25%). Verify calculation."
                    )
                else:
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  - WACC in reasonable range for Indian markets (8-25%)")
