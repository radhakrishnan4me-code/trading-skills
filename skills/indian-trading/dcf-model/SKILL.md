---
name: dcf-model
description: Real DCF (Discounted Cash Flow) model creation for equity valuation. Retrieves financial data from BSE/NSE corporate filings and analyst reports, builds comprehensive cash flow projections with proper WACC calculations, performs sensitivity analysis, and outputs professional Excel models with executive summaries. Use when users need to value a company using DCF methodology, request intrinsic value analysis, or ask for detailed financial modeling with growth projections and terminal value calculations.
---
...
- Verify tax rate is reasonable (typically 25-35% for Indian companies)
...
Where:
- Risk-Free Rate = Current 10-year India G-Sec yield (typically 7.0-7.5%)
- Beta = 5-year monthly stock beta vs Nifty 50
- India ERP = 7.0-8.0% (includes country risk premium)
```
...
**Typical WACC Ranges (India):**
- Large Cap, Stable: 11-13%
- Growth Companies: 13-16%
- High Growth/Risk: 16-20%
...
**Terminal Growth Rate Selection (India):**
- Conservative: 4.0-5.0% (below India nominal GDP growth)
- Moderate: 5.0-6.0% (aligned with India nominal GDP growth)
- Aggressive: 6.0-7.0% (only for market leaders in high-growth sectors)

**Do not exceed**: Risk-free rate (India G-Sec yield) or long-term India nominal GDP growth
...
```
(+) Sum of PV of Projected FCFs = ₹X Cr
(+) PV of Terminal Value = ₹Y Cr
= Enterprise Value = ₹Z Cr

(-) Net Debt [or + Net Cash if negative] = ₹A Cr
= Equity Value = ₹B Cr

÷ Diluted Shares Outstanding = C Cr shares
...
```csv
Item,Source Comment
Stock price,Source: NSE/BSE market data 2025-10-12 Close price
Shares outstanding,Source: Annual Report FY25 Note 12
Historical revenue,Source: Annual Report FY25 Consolidated Statement of Profit and Loss
Beta,Source: Screener.in / Moneycontrol 2025-10-12 5-year monthly beta vs Nifty 50
Consensus estimates,Source: Management guidance Q3 FY25 earnings call
```
...
`CapEx: =E29*$E$35   // Should be $E$22, but row shifted`
...
- Incorrect risk-free rate (must use current 10Y India G-Sec)
...
Example:
```bash
python recalc.py TCS_DCF_Model_2025-10-12.xlsx 30
```
...
- **Years**: Format as text strings (e.g., "FY25" not "2,025")
- **Percentages**: `0.0%` (one decimal place)
- **Currency**: `₹#,##0` for crores; `₹#,##0.00` for per-share - ALWAYS specify units in headers ("Revenue (₹ Cr)")
...
Shares Outstanding (Cr),XX.X
Market Cap (₹ Cr),[Formula]
Net Debt (₹ Cr),XXX [or Net Cash if negative]
```
...
```csv
Income Statement (₹ Cr),FY21A,FY22A,FY23A,FY24A,FY25E,FY26E,FY27E
...
```csv
Cash Flow (₹ Cr),FY21A,FY22A,FY23A,FY24A,FY25E,FY26E,FY27E
...
```csv
DCF Valuation,FY25E,FY26E,FY27E,FY28E,FY29E,Terminal
Unlevered FCF (₹ Cr),XXX,XXX,XXX,XXX,XXX,
Period,0.5,1.5,2.5,3.5,4.5,
Discount Factor,0.XX,0.XX,0.XX,0.XX,0.XX,
PV of FCF (₹ Cr),XXX,XXX,XXX,XXX,XXX,
,,,,,,
Terminal FCF (₹ Cr),,,,,,,XXX
Terminal Value (₹ Cr),,,,,,,XXX
PV Terminal Value (₹ Cr),,,,,,,XXX
,,,,,,
Valuation Summary (₹ Cr),,,,,,
Sum of PV FCFs,XXX,,,,,
...
Risk-Free Rate (10Y India G-Sec),7.XX%,[Yellow input]
Beta (5Y monthly vs Nifty 50),X.XX,[Yellow input]
India Equity Risk Premium (incl. CRP),7.XX%,[Yellow input]
...
Shares Outstanding (Cr),XX.X,[Link to DCF]
Market Capitalization (₹ Cr),"X,XXX",[Calculated]
,,
Total Debt (₹ Cr),XXX,[Yellow input]
Cash & Equivalents (₹ Cr),XXX,[Yellow input]
Net Debt (₹ Cr),XXX,[Calculated]
,,
Enterprise Value (₹ Cr),"X,XXX",[Calculated]
,,
...
- India nominal GDP-aligned terminal growth (5.0-6.0%)
...
2. **Gather historical financials**:
   ...
   - Manual extraction from Annual Reports / BSE/NSE filings if necessary
...
- **Manual extraction**: BSE/NSE corporate filings, Screener.in, Trendlyne as fallback
...
- Tax rate 25-35% (Indian corporate tax rates)
- File naming: `[Ticker]_DCF_Model_[Date].xlsx`