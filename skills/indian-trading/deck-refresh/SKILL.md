---
name: deck-refresh
description: Updates a presentation with new numbers — quarterly refreshes, earnings updates, comp rolls, rebased market data. Use whenever the user asks to "update the deck with Q4 numbers", "refresh the comps", "roll this forward", "swap in the new earnings", "change all the ₹485 Cr to ₹512 Cr", or any request to swap figures across an existing deck without rebuilding it.
---
...
- **Pasted mapping** — user types or pastes "revenue ₹485 Cr → ₹512 Cr, EBITDA ₹120 Cr → ₹135 Cr." The clearest case.
...
- **Just the new values** — "Q4 revenue was ₹512 Cr, margins were 22%." You figure out what each one replaces. Workable, but confirm the mapping before you touch anything — a "₹512 Cr" that you map to revenue but the user meant for gross profit is a quiet disaster.
...
| Variant | Example |
|---|---|
| Scale | `₹485 Cr`, `₹4,850 L`, `₹4,85,00,00,000` |
| Precision | `₹485 Cr`, `₹485.0 Cr`, `~₹485 Cr` |
| Unit style | `₹485 Cr`, `₹485 crores`, `485 Cr` |
| Embedded | "revenue grew to ₹485 Cr", "a ₹485 Cr business", axis labels |

A deck that says `₹485 Cr` on slide 3, `485` on slide 8's chart axis, and `₹485.0 crores` in a footnote on slide 15 has three instances of the same number. Find-replace misses two of them. You shouldn't.
...
```
₹485 Cr → ₹512 Cr (Revenue)
  Slide 3  — Title box: "Revenue grew to ₹485 Cr"
  Slide 8  — Chart axis label: "485"
  Slide 15 — Footnote: "₹485.0 crores in FY25 revenue"

₹120 Cr → ₹135 Cr (Adj. EBITDA)
  Slide 3  — Table cell
  Slide 11 — Body text: "₹120 Cr of Adj. EBITDA"

FLAGGED — possibly derived, not in your mapping:
  Slide 3  — "+15% YoY" (growth rate — stale if base year didn't change?)
  Slide 7  — "12% market share" (was this computed from ₹485 Cr / market size?)
```

The flagged section matters. You're not just executing a find-replace — you're catching the second-order effects the user would've missed at 11pm. If the mapping says `₹485 Cr → ₹512 Cr` and slide 3 also has `+15% YoY` right next to it, that growth rate is probably wrong now. Flag it; don't silently fix it, don't silently leave it.
...
- **Text in a shape** — change the value, leave font/size/color/bold state exactly as they were. If `₹485 Cr` is 14pt navy bold inside a sentence, `₹512 Cr` is 14pt navy bold inside the same sentence.
...
Run standard visual verification checks on every edited slide. A number that got longer (`₹485 Cr` → `₹1,205 Cr`) might now overflow its text box or push a table column width. Catch it before the user does.
...
- **Not touching formatting** — if the deck uses `₹ Cr` and the user's mapping says `₹ L`, match the deck, not the mapping. Values change; style stays.
