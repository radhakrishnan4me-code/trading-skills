...
3. **ONLY if MCPs are unavailable:** Then use Bloomberg Terminal, BSE/NSE corporate filings, or other institutional sources
...
Row 3: As of [Period] | All figures in [INR Crores] except per-share amounts and ratios
```
...
- **Decimal precision**:
  ...
  - Rupee amounts: No decimals, thousands separator (69,632)
...
4. **EV/Revenue** - How much market pays per rupee of sales
5. **EV/EBITDA** - How much market pays per rupee of earnings
...
- Where did the data come from? (S&P Kensho MCP, FactSet MCP, Daloopa MCP, Bloomberg, BSE/NSE filings)
- What period does it cover? (Q4 2024, audited figures)
- How was it verified? (Cross-checked against Annual Report / Quarterly Results)
...
→ Skip: Size metrics, absolute rupee amounts
...
3. **Standardize units upfront** - Lakhs vs. crores decision affects everything
...
2. **Add cell comments to ALL hard-coded inputs** - Right-click cell → Insert Comment → Document source OR assumption
   ...
   **For sourced data, cite exactly where it came from:**
   - Example: "Screener.in - TCS financials, accessed 2025-03-10"
   - Example: "FY25 Annual Report, page 42, line item 'Revenue from Operations'"
   - Example: "FactSet consensus estimate as of 2024-10-02"
   - **Include hyperlinks when possible**: Right-click cell → Link → paste URL to BSE/NSE filing, data source, or report

   **For assumptions, explain the reasoning:**
   - Example: "Assumed 15% EBITDA margin based on peer median, company does not disclose"
   - Example: "Estimated Enterprise Value as Market Cap + ₹500 Cr net debt (from Q3 balance sheet, Q4 not yet available)"
   - Example: "Forward P/E based on street consensus EPS of ₹45.50 (average of 12 analyst estimates)"
...
❌ Missing hyperlinks to BSE/NSE filings or data sources when available
...
2. **Gather data** (60-90 minutes)
   - Pull from primary sources (S&P Kensho MCP, FactSet MCP, Daloopa MCP if available; otherwise Bloomberg, BSE/NSE filings, Screener.in, Trendlyne)
...
- **Link to source files**: Hyperlink to BSE/NSE filings, Screener.in, or Trendlyne pages
...
- [ ] **Hyperlinks added to cells where applicable** (BSE/NSE filings, data provider pages, reports)
...
```
┌─────────────────────────────────────────────────────────────┐
│ IT SERVICES - COMPARABLE COMPANY ANALYSIS                   │
│ TCS • Infosys • Wipro                                       │
│ As of Q4 FY25 | All figures in ₹ Crores                    │
├─────────────────────────────────────────────────────────────┤
│ OPERATING METRICS                                           │
├──────────┬─────────┬─────────┬──────────┬──────────────────┤
...
├──────────┼─────────┼─────────┼──────────┼─────────┼────────┤
│ TCS      │ 259,000 │ 4.1%    │ 42.5%    │ 75,100  │ 29.0%  │
│ INFY     │ 162,000 │ 5.8%    │ 38.2%    │ 43,800  │ 27.0%  │
│ WIPRO    │  91,000 │ 1.2%    │ 30.5%    │ 18,200  │ 20.0%  │
│          │         │         │          │         │        │ [blank row]
...
├──────────┼──────────┼──────────┼──────────┼───────────┼────┤
│ TCS      │1,450,000 │1,440,000 │  5.6x    │ 19.2x     │32.0│
│ INFY     │  720,000 │  710,000 │  4.4x    │ 16.2x     │28.5│
│ WIPRO    │  310,000 │  305,000 │  3.4x    │ 16.8x     │26.3│
│          │          │          │          │           │    │ [blank row]
...
- [ ] Units are clearly labeled (lakhs/crores)
...
- [ ] **Hyperlinks added where relevant** (BSE/NSE corporate filings, Bloomberg pages, research reports)
