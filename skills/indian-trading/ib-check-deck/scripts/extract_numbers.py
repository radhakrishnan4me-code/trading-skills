...
def extract_numbers(content: str) -> list[NumberInstance]:
    ...
    # Matches: ₹500 Cr, 500 Cr, ₹500 crores, 25%, 25.5%, 2.5x, 150bps, ₹1,234.56, $500M, etc.
    number_pattern = re.compile(
        r'(?P<currency>[$€£¥₹])?'  # Optional currency symbol (includes ₹ for INR)
        ...
        r'[Cc]r|[Cc]rores?|[Ll]akhs?|[Ll])?'  # Indian units
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        for match in number_pattern.finditer(line):
            ...
            if currency:
                if currency == '₹':
                    if not unit:
                        unit = 'INR'  # Assume INR for ₹ without unit
                    else:
                        unit = f"INR_{unit}"
                else:
                    if not unit:
