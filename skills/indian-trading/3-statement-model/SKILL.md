...
- **If running inside Excel (Office Add-in / Office JS):** Use Office JS directly. Write formulas via `range.formulas = [["=D14*(1+Assumptions!₹X,XXX Cr$5)"]]` — never `range.values` for derived cells. No separate recalc; Excel computes natively. Use `context.workbook.worksheets.getItem(...)` to navigate tabs.
- **If generating a standalone .xlsx file:** Use Python/openpyxl. Write `ws["D15"] = "=D14*(1+Assumptions!₹X,XXX Cr$5)"`, then run `recalc.py` before delivery.
...
- When using Python/openpyxl: write formula strings (`ws["D15"] = "=D14*(1+Assumptions!₹X,XXX Cr$5)"`), NOT computed results (`ws["D15"] = 12500`)
...
- Find the units row indicating ₹ crores, %, x, etc.
...
## Indian Corporate Filings Data Extraction

If the template specifically requires pulling data from BSE/NSE corporate filings (Annual Reports, Quarterly Results under LODR Regulation 33), see [references/sec-filings.md](references/sec-filings.md) for detailed extraction guidance. This reference is only needed when populating templates with public company data from regulatory filings.
...
| Match the template's units | Verify if template uses lakhs, crores, or actual values before entering data |
...
- NOL utilization limited per Indian Income Tax Act provisions (carry-forward up to 8 years for business losses)
